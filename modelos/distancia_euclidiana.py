...
# --- Diferencia numérica normalizada (para distancia euclidiana) ---
def diff_numeric(a, b, key):
    """
    Devuelve la diferencia (na - nb) en el espacio normalizado [0,1].
    Si falta algún valor, devuelve None.
    """
...
# *************** Similitud ponderada (EUCLIDIANA)

def similarity_weighted_A(qvec, cvec, domain):
    """
    Calcula similitud a partir de distancia euclidiana ponderada
    sobre variables normalizadas en [0,1].

    - Para variables numéricas: se usa diff = na - nb (normalizadas) y se acumula diff^2.
    - Para la fase lunar: se usa sim_luna, se convierte a distancia (1 - sim)
      y también se incluye como componente en la distancia.
    """
    ...
    for k, w in weights.items():
        ...
        if k == "luna_fase":
            simk = sim_luna(qv, cvec.get("luna_fase"))
            if simk is None:
                continue
            diff = 1.0 - clip01(simk)
        else:
            d = diff_numeric(qv, cv, k)
            if d is None:
                continue
            diff = d

        sum_w += w
        sum_w_sqdiff += w * (diff ** 2)

    if sum_w <= 0:
        return 0.0

    dist = math.sqrt(sum_w_sqdiff / sum_w)
    return clip01(1.0 - dist)

def similarity_weighted_B(qvec, cvec, query_alt):
    """
    Similaridad basada en distancia euclidiana ponderada (Dataset B),
    luego ajustada por el bono de estación.
    """
    sum_w = 0.0
    sum_w_sqdiff = 0.0
