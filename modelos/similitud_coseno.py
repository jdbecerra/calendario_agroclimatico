...
# (sim_numeric queda obsoleto, se deja solo por compatibilidad)
...
# *************** Similitud ponderada (COSENO)

def similarity_weighted_A(qvec, cvec, domain):
    """
    Similitud de coseno ponderada en el espacio normalizado [0,1]
    para las variables numéricas, combinada con la similitud de fase
    lunar según su peso.

    - Para variables numéricas: se normalizan (0-1) y se aplica coseno
      con pesos WEIGHTS[domain].
    - Para luna_fase: se calcula sim_luna (0–1) y se combina con el
      coseno numérico usando el peso de luna_fase.
    """
    weights = WEIGHTS[domain]

    # Parte numérica (todas las claves excepto luna_fase)
    sum_w_qc = 0.0
    sum_w_q2 = 0.0
    sum_w_c2 = 0.0
    sum_w_num = 0.0
    ...
    if sum_w_num > 0 and sum_w_q2 > 0 and sum_w_c2 > 0:
        cos_num = sum_w_qc / (math.sqrt(sum_w_q2) * math.sqrt(sum_w_c2))
        cos_num = clip01(cos_num)
    else:
        cos_num = None

    # Parte lunar (categórica)
    sim_l = None
    w_luna = 0.0
    if "luna_fase" in weights:
        q_luna = qvec.get("luna_fase")
        c_luna = cvec.get("luna_fase")
        if q_luna and c_luna:
            sim_l = sim_luna(q_luna, c_luna)
            if sim_l is not None:
                sim_l = clip01(sim_l)
                w_luna = weights["luna_fase"]

    # Combinar coseno numérico + luna según pesos
    if cos_num is not None and sim_l is not None and w_luna > 0 and sum_w_num > 0:
        sim_total = (sum_w_num * cos_num + w_luna * sim_l) / (sum_w_num + w_luna)
    elif cos_num is not None:
        sim_total = cos_num
    elif sim_l is not None:
        sim_total = sim_l
    else:
        sim_total = 0.0

    return clip01(sim_total)

def similarity_weighted_B(qvec, cvec, query_alt):
    """
    Similaridad de coseno ponderada (Dataset B) sobre variables numéricas
    normalizadas, luego ajustada por el bono de estación.
    """
    sum_w_qc = 0.0
    sum_w_q2 = 0.0
    sum_w_c2 = 0.0
    ...
    if sum_w > 0 and sum_w_q2 > 0 and sum_w_c2 > 0:
        cos_sim = sum_w_qc / (math.sqrt(sum_w_q2) * math.sqrt(sum_w_c2))
        base_sim = clip01(cos_sim)
    else:
        base_sim = 0.0

    bonus = station_bonus(query_alt, cvec.get("estacion"), cvec.get("altitud_msnm"))
    return clip01(base_sim * bonus)
...
def run_cbr(params):
    ...
    fase_actual = fase_in or inferir_fase(params.get("altitud"),
                                          params.get("mes"),
